...
void VulkanEngine::draw()
{
	...
	VK_CHECK(vkWaitForFences(_device, 1, &current_frame_data._renderFence, true, 1000000000));
	VK_CHECK(vkResetFences(_device, 1, &current_frame_data._renderFence));

	//now that we are sure that the commands finished executing, we can safely reset the command buffer to begin recording again.
	VK_CHECK(vkResetCommandBuffer(current_frame_data._mainCommandBuffer, 0));

	//request image from the swapchain, one second timeout
	uint32_t swapchainImageIndex;
	VK_CHECK(vkAcquireNextImageKHR(_device, _swapchain, 1000000000, current_frame_data._presentSemaphore, nullptr, &swapchainImageIndex));

	//naming it cmd for shorter writing
	auto& cmd = current_frame_data._mainCommandBuffer;
	...
	submit.pWaitSemaphores = &current_frame_data._presentSemaphore;

	submit.signalSemaphoreCount = 1;
	submit.pSignalSemaphores = &current_frame_data._renderSemaphore;

	//submit command buffer to the queue and execute it.
	// _renderFence will now block until the graphic commands finish execution
	VK_CHECK(vkQueueSubmit(_graphicsQueue, 1, &submit, current_frame_data._renderFence));
	...
	presentInfo.pWaitSemaphores = &current_frame_data._renderSemaphore;
...
void VulkanEngine::init_vulkan()
{
	...
	auto inst_ret = builder.set_app_name("vktest")
...
void VulkanEngine::init_descriptors()
{
	...
	VkDescriptorSetLayoutBinding texturedBind = vkinit::descriptorset_layout_binding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 0);
	...
	set3info.pBindings = &texturedBind;
